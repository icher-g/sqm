package io.sqm.render.ansi;

import io.sqm.core.Predicate;
import io.sqm.render.ansi.spi.AnsiDialect;
import io.sqm.render.spi.RenderContext;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Nested;
import org.junit.jupiter.api.Test;

import static io.sqm.dsl.Dsl.col;
import static io.sqm.dsl.Dsl.row;
import static org.junit.jupiter.api.Assertions.*;

/**
 * Unit tests for {@link ComparisonPredicateRenderer}.
 */
class ComparisonPredicateRendererTest {

    private String render(Predicate p) {
        var ctx = RenderContext.of(new AnsiDialect());
        return ctx.render(p).sql();
    }

    @Test
    void eq_single() {
        Predicate p = col("t", "c").eq("X");
        var result = render(p);
        assertEquals("t.c = 'X'", result);
    }

    @Test
    void eq_column() {
        Predicate p = col("t", "c").eq(col("v", "d"));
        var result = render(p);
        assertEquals("t.c = v.d", result);
    }

    // --- Simple binary/unary operators (single value) ---

    @Test
    void ne_single() {
        Predicate p = col("col").ne(42);
        var result = render(p);
        assertEquals("col <> 42", result);
    }

    @Test
    void lt_lte_gt_gte_like_notLike_single() {
        var ctx = RenderContext.of(new AnsiDialect());

        var w1 = ctx.render(col("c").lt(10));
        assertEquals("c < 10", w1.sql());

        var w2 = ctx.render(col("c").lte(11));
        assertEquals("c <= 11", w2.sql());

        var w3 = ctx.render(col("c").gt(12));
        assertEquals("c > 12", w3.sql());

        var w4 = ctx.render(col("c").gte(13));
        assertEquals("c >= 13", w4.sql());

        var w5 = ctx.render(col("c").like("%abc%"));
        assertEquals("c LIKE '%abc%'", w5.sql());

        var w6 = ctx.render(col("c").notLike("%abc%"));
        assertEquals("c NOT LIKE '%abc%'", w6.sql());
    }

    // --- IN / NOT IN ---

    @Test
    void in_list_vals() {
        var f = col("c").in(1, 2, 3);
        var result = render(f);
        assertEquals("c IN (1, 2, 3)", result);
    }

    @Test
    @DisplayName("NOT IN with a single value downgrades to '<>'")
    void notIn_single_downgrades_to_ne() {
        var f = col("c").ne(7);
        var result = render(f);
        assertEquals("c <> 7", result);
    }

    @Test
    void notIn_list_vals() {
        var f = col("c").notIn("a", "b");
        var result = render(f);
        assertEquals("c NOT IN ('a', 'b')", result);
    }

    @Test
    void range_between() {
        var f = col("c").between(5, 10);
        var result = render(f);
        assertEquals("c BETWEEN 5 AND 10", result);
    }

    // --- Range / Nullability ---

    @Test
    void is_null_ignores_value() {
        var f = col("c").isNull();
        var result = render(f);
        assertEquals("c IS NULL", result);
    }

    @Test
    void is_not_null_ignores_value() {
        var f = col("c").isNotNull();
        var result = render(f);
        assertEquals("c IS NOT NULL", result);
    }

    @Nested
    @DisplayName("Supported operators")
    class SupportedOperators {

        @Test
        void rendersEq() {
            var left = col("orders", "customer_id");
            var right = col("customers", "id");
            var jf = left.eq(right);

            var text = render(jf);

            assertEquals("orders.customer_id = customers.id", text);
        }

        @Test
        void rendersNe() {
            var left = col("a", "x");
            var right = col("b", "y");
            var jf = left.ne(right);

            var text = render(jf);

            assertEquals("a.x <> b.y", text);
        }

        @Test
        void rendersLt() {
            var jf = col("t1", "c1").lt(col("t2", "c2"));
            var text = render(jf);

            assertEquals("t1.c1 < t2.c2", text);
        }

        @Test
        void rendersLte() {
            var jf = col("t1", "c1").lte(col("t2", "c2"));
            var text = render(jf);

            assertEquals("t1.c1 <= t2.c2", text);
        }

        @Test
        void rendersGt() {
            var jf = col("t1", "c1").gt(col("t2", "c2"));
            var text = render(jf);

            assertEquals("t1.c1 > t2.c2", text);
        }

        @Test
        void rendersGte() {
            var jf = col("t1", "c1").gte(col("t2", "c2"));
            var text = render(jf);

            assertEquals("t1.c1 >= t2.c2", text);
        }
    }

    // --- Error branches ---

    @Nested
    class ErrorCases {
        @Test
        void eq_with_list_throws() {
            IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () -> col("c").eq(row(1, 2)));
            assertTrue(ex.getMessage().contains("EQ"));
        }
    }
}
