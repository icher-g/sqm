...
/**
 * Renders PostgreSQL sequence value expressions.
 */
public class SequenceValueExprRenderer extends io.sqm.render.ansi.SequenceValueExprRenderer {
    /**
     * Creates a PostgreSQL sequence value expression renderer.
     */
    ...
    public void render(SequenceValueExpr node, RenderContext ctx, SqlWriter w) {
        ...
        w.append(node.kind() == SequenceValueKind.NEXT_VALUE ? "nextval(" : "currval(")
            .append("'").append(String.join(".", node.sequence().values()).replace("'", "''")).append("'")
            .append(")");
    }
}
