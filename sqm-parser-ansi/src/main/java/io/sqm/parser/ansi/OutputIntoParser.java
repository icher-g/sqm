 ...
 */
public class OutputIntoParser implements Parser<OutputInto> {

    /**
     * Creates an output-into parser.
     */
    public OutputIntoParser() {
    }
     ...
     */
    @Override
    public ParseResult<OutputInto> parse(Cursor cur, ParseContext ctx) {
        ...
        if (cur.consumeIf(TokenType.LPAREN)) {
            columns = parseIdentifierItems(cur, "Expected output target column");
            cur.expect("Expected ) after OUTPUT INTO target columns", TokenType.RPAREN);
        }

        return ok(OutputInto.of(table.value(), columns));
    }
