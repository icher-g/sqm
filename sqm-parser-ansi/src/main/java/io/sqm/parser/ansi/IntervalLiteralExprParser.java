...
/**
 * Parses {@code INTERVAL '...'} literals with optional qualifiers.
 */
public class IntervalLiteralExprParser implements MatchableParser<IntervalLiteralExpr> {
    ...
    public ParseResult<IntervalLiteralExpr> parse(Cursor cur, ParseContext ctx) {
        if (!ctx.capabilities().supports(SqlFeature.INTERVAL_LITERAL)) {
            return error("INTERVAL literals are not supported by this dialect", cur.fullPos());
        }
