...
class ComprehensiveRecursiveNodeVisitorTest {
    ...
    void visitWithPredicateLogic() {
        ...
        Predicate predicate = col("id").eq(1)
    ...
    void visitComparisonPredicate() {
        ComparisonPredicate predicate = col("id").eq(lit(1));
        CountingVisitor visitor = new CountingVisitor();
        predicate.accept(visitor);
    ...
    void visitBetweenPredicate() {
        BetweenPredicate predicate = col("age").between(lit(18), lit(65));
        CountingVisitor visitor = new CountingVisitor();
        predicate.accept(visitor);
    ...
    void visitInPredicate() {
        InPredicate predicate = col("status").in(row("active", "pending", "review"));
        CountingVisitor visitor = new CountingVisitor();
        predicate.accept(visitor);
    ...
    void visitLikePredicate() {
        LikePredicate predicate = col("email").like("%@example.com");
        CountingVisitor visitor = new CountingVisitor();
        predicate.accept(visitor);
    ...
    void visitIsNullPredicate() {
        IsNullPredicate predicate = col("optional_field").isNull();
        CountingVisitor visitor = new CountingVisitor();
        predicate.accept(visitor);
    ...
    @Test
    void visitCastExpr() {
        CastExpr cast = col("id").cast(type("VARCHAR"));
        CountingVisitor visitor = new CountingVisitor();
        cast.accept(visitor);
