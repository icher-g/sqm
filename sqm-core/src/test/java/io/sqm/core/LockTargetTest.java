...
class LockTargetTest {
    ...
    void createWithIdentifier() {
        var target = LockTarget.of("users");
        
        assertNotNull(target);
        assertEquals("users", target.identifier());
    }

    @Test
    @DisplayName("Create lock target with table alias")
    void createWithAlias() {
        var target = LockTarget.of("u");
        
        assertNotNull(target);
        assertEquals("u", target.identifier());
    }
    ...
    void equalityWorks() {
        var target1 = LockTarget.of("users");
        var target2 = LockTarget.of("users");
        var target3 = LockTarget.of("orders");
