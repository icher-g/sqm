...
class CursorTest {
    ...
    void of_createsCursorFromString() {
        Cursor cur = Cursor.of("SELECT * FROM t");
        assertNotNull(cur);
    ...
    void peek_returnsCurrentToken() {
        Cursor cur = Cursor.of("SELECT * FROM");
        assertEquals(TokenType.SELECT, cur.peek().type());
    ...
    void find_findsTokenAtTopLevel() {
        Cursor cur = Cursor.of("SELECT * FROM t WHERE x = 1");
    ...
    void find_skipsNestedParentheses() {
        Cursor cur = Cursor.of("SELECT * FROM (SELECT x, y FROM t) WHERE z = 1");
        int idx = cur.find(TokenType.WHERE);
        assertTrue(idx < cur.size());
    ...
    void find_withLookahead() {
        Cursor cur = Cursor.of("SELECT * FROM t WHERE");
    ...
    void find_withCustomSkipTokens() {
        ...
        Cursor cur = Cursor.of("SELECT CASE WHEN x THEN y END FROM t");
    ...
    void find_respectsNestedDepth() {
        Cursor cur = Cursor.of("SELECT (a, (b, c), d) FROM t");
