...
- Oracle query support includes modeled hierarchical queries (`CONNECT BY`) plus baseline `PIVOT` / `UNPIVOT` relation transforms in parser, renderer, validation, codegen, and JSON. The hierarchical slice also includes recursive-CTE transpilation for the supported simple parent-child subset.
