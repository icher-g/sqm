...
- Oracle support includes its 19c query/DML baseline, `RETURNING ... INTO`, baseline `MERGE`, and Oracle-aware catalog type mapping. R11 additionally ships modeled hierarchical queries (`CONNECT BY`) and baseline `PIVOT` / `UNPIVOT` relation transforms; the hierarchy slice includes conservative recursive-CTE transpilation, while pivot/unpivot provides opt-in approximate rewrites for simple top-level transforms.
